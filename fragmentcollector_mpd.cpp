...
void StreamAbstractionAAMP_MPD::ProcessMetadataFromManifest( ManifestDownloadResponsePtr mpdDnldResp, bool init)
{
	...
	if (tmpMPD)
	{
		...
		if (ISCONFIGSET(eAAMPConfig_DetectEarlyAvailablePeriod) && mIsLiveManifest)
		{
 ...
 */
void StreamAbstractionAAMP_MPD::ProcessEarlyAvailablePeriod(ManifestDownloadResponsePtr mpdDnldResp)
{
	if (!mpdDnldResp)
	{
		return;
...
void StreamAbstractionAAMP_MPD::GetEarlyAvailablePeriods(std::vector<IPeriod*>& PeriodIds, AampMPDParseHelperPtr mpdParseHelper)
{
   ...
   if (!adaptationSets.empty() && mpdParseHelper->IsEmptyPeriod(periodIter, false))
   {
	   if (std::find(mEarlyAvailablePeriodIds.begin(), mEarlyAvailablePeriodIds.end(), tempPeriod->GetId()) == mEarlyAvailablePeriodIds.end())
	   {
